...
losses = np.zeros((100,3),dtype=np.int)
training_loss = 0
validation_loss = 0
...
def train_model(model, output_uri, dataloader, loss_function, optimizer, scheduler, epochs, val_dataloader, intervals, input_size, num_kpt, save_checkpoints, kpt_keys, study_name, evaluate_mode):
    ...
    for epoch in range(epochs):
        ...
        training_loss = round(loss,4)*100000
        validation_loss = round(val_loss,4)*100000
        losses[epoch,0] = epoch
        losses[epoch,1] = training_loss
        losses[epoch,2] = validation_loss
        ...
        if tolerance >= max_tolerance:
            ...
            np.savetxt('loss_epoch.dat', losses, delimiter='  ')
            break
